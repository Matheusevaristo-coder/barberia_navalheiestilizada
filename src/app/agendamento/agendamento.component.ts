...
export class AgendamentoComponent {
  ...
  validarAgendamento(): boolean {
    ...
    if (dataSelecionada < this.minDate) {
      this.mostrarErro('Não é possível agendar para uma data passada');
      return false;
    }

    // Validação adicional para o horário
    const horarioSelecionado = form.get('horario')?.value;
    if (!this.horariosDisponiveis.includes(horarioSelecionado)) {
      this.mostrarErro('Horário selecionado não está disponível');
      return false;
    }
